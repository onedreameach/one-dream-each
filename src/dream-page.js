...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .home-link{position:relative;isolation:isolate;min-width:116px;height:54px;padding:5px 14px 5px 6px;border-radius:20px;display:flex;align-items:center;justify-content:flex-start;gap:10px;text-decoration:none;color:#eef3f7;background:linear-gradient(145deg,rgba(13,25,38,.98),rgba(7,12,22,.99));border:1px solid rgba(139,202,226,.18);box-shadow:0 0 0 2px rgba(0,0,0,.55),0 12px 30px rgba(0,0,0,.20),inset 0 1px 0 rgba(255,255,255,.055);overflow:hidden}
    .home-link:before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 18% 50%,rgba(90,225,238,.12),transparent 36%),radial-gradient(circle at 78% 30%,rgba(167,116,255,.10),transparent 42%);z-index:-1;pointer-events:none}
    .home-icon{flex:0 0 42px;width:42px;height:42px;border-radius:14px;display:grid;place-items:center;background:linear-gradient(145deg,rgba(62,198,220,.22),rgba(133,87,223,.26));border:1px solid rgba(150,211,232,.17);box-shadow:inset 0 1px 0 rgba(255,255,255,.07),0 0 20px rgba(116,112,255,.09)}
    .home-icon svg{width:24px;height:24px}
    .home-copy{display:flex;flex-direction:column;align-items:flex-start;justify-content:center;line-height:1}
    .home-copy small{display:block;color:#6f8291;font-size:5.2px;font-weight:900;letter-spacing:.15em}
    .home-copy b{display:block;margin-top:5px;color:#f2f6f8;font:800 10px/1 "Space Grotesk",Inter,sans-serif;letter-spacing:.07em}
    ...
    @media(max-width:760px){
      ...
      .home-link{flex:0 0 104px;min-width:104px;width:104px;height:48px;padding:4px 10px 4px 5px;border-radius:18px;gap:8px}.home-icon{flex-basis:38px;width:38px;height:38px;border-radius:13px}.home-icon svg{width:22px;height:22px}.home-copy small{display:none}.home-copy b{font-size:9px;margin:0;letter-spacing:.08em}
...
<body>
  ...
  (function(){
    ...
    document.getElementById("share-card")?.addEventListener("click",async function(){try{setStatus("Preparing your Dream Card...");const file=await getCardFile();if(navigator.share&&navigator.canShare&&navigator.canShare({files:[file]})){await navigator.share({title:"Dream #"+padded+" — OneDreamEach",text:"One real person. One Dream. One permanent place.",files:[file]});setStatus("Dream shared.");}else if(navigator.share){await navigator.share({title:"Dream #"+padded+" — OneDreamEach",text:"One real person. One Dream. One permanent place.",url:dreamUrl});setStatus("Dream page shared.");}else{downloadBlob(file);setStatus("Dream Card downloaded — share it anywhere.");}}catch(e){if(e&&e.name==="AbortError")setStatus("");else{console.error(e);setStatus("Unable to share right now.");}}});
...
function rocketIcon(){return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14.8 4.2c2.6-.8 4.5-.5 5-.2.3.5.6 2.4-.2 5-1 3.2-3.4 5.4-6.1 7.1l-3.6-3.6c1.6-2.7 3.8-5.2 6.9-6.2Z" fill="#8fb5ff" stroke="#c5a8ff" stroke-width=".8"/><circle cx="16.5" cy="7.5" r="2" fill="#70e6ef"/><path d="m9.9 12.5-3.5.4-2.2 2.2 4.5.7M13.5 16.1l-.4 3.5-2.2 2.2-.7-4.5" fill="#7ce8ef" opacity=".9"/><path d="M8.3 16.6 5 20" stroke="#e78ed1" stroke-width="1.8" stroke-linecap="round"/></svg>';}
