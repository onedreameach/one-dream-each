...
<body>
  ...
  (function(){
    ...
    async function buildCardBlob(){
      ...
      /* Official 9:16 card — intentionally simple, serious and recognisable. */
      const bg=x.createLinearGradient(0,0,1080,1920);bg.addColorStop(0,"#070C12");bg.addColorStop(.62,"#09111A");bg.addColorStop(1,"#0D0D18");x.fillStyle=bg;x.fillRect(0,0,1080,1920);
      const light=x.createRadialGradient(140,120,0,140,120,720);light.addColorStop(0,"rgba(72,211,225,.13)");light.addColorStop(1,"rgba(72,211,225,0)");x.fillStyle=light;x.fillRect(0,0,820,780);
      const violet=x.createRadialGradient(1020,1820,0,1020,1820,680);violet.addColorStop(0,"rgba(130,108,230,.11)");violet.addColorStop(1,"rgba(130,108,230,0)");x.fillStyle=violet;x.fillRect(350,1160,730,760);

      const fx=48,fy=48,fw=984,fh=1824;
      x.save();x.shadowColor="rgba(0,0,0,.58)";x.shadowBlur=55;x.shadowOffsetY=18;rr(fx,fy,fw,fh,52,"#090F17");x.restore();
      rr(fx,fy,fw,fh,52,"#090F17","rgba(151,179,196,.20)");
      const topLine=x.createLinearGradient(82,0,998,0);topLine.addColorStop(0,"#58DCE6");topLine.addColorStop(.55,"#7E9DE8");topLine.addColorStop(1,"#AE86E6");x.fillStyle=topLine;rr(82,82,916,4,2,topLine);

      /* Brand and identity */
      txt("ONEDREAMEACH",84,140,"900 18px Inter, Arial","#DDE8EC");
      txt("OFFICIAL DREAM CARD",996,140,"900 11px Inter, Arial","#71838E","right");
      line(84,174,996,174,"rgba(255,255,255,.08)",2);
      const num=x.createLinearGradient(84,0,620,0);num.addColorStop(0,"#69DFE8");num.addColorStop(.55,"#9AABEF");num.addColorStop(1,"#C291E6");
      txt("#"+padded,84,300,"900 108px Space Grotesk, Inter, Arial",num);
      txt("1 OF 1,000,000",996,270,"900 13px Inter, Arial","#8396A1","right");
      txt("PERMANENT · PUBLIC",996,300,"900 12px Inter, Arial","#B2C0C7","right");

      /* Origin */
      rr(84,350,912,154,25,"rgba(5,12,19,.62)","rgba(255,255,255,.07)");
      txt("ORIGIN",112,396,"900 11px Inter, Arial","#718A96");
      const countryFit=fitCountry(dream.country,620);const cSize=Math.min(countryFit.size,58);x.font="900 "+cSize+"px Space Grotesk, Inter, Arial";const cLines=wrapText(String(dream.country||"World").toUpperCase(),620).slice(0,2);let cy=458-(cLines.length-1)*24;cLines.forEach((ln,i)=>txt(ln,110,cy+i*52,"900 "+cSize+"px Space Grotesk, Inter, Arial","#F4F7F8"));
      const flag=await loadFlag();rr(824,382,138,88,18,"#0C141D","rgba(255,255,255,.09)");if(flag){x.save();rr(832,390,122,72,13,"#0C141D");x.clip();contain(flag,838,396,110,60);x.restore();if(flag.__url)URL.revokeObjectURL(flag.__url)}else{txt(dream.countryCode||"WORLD",893,435,"900 15px Inter, Arial","#C9D5DA","center")}

      /* The Dream — the only real hero. */
      txt("THE DREAM",86,580,"900 11px Inter, Arial","#788D98");
      line(84,607,996,607,"rgba(255,255,255,.07)",2);
      line(116,665,116,1265,"rgba(104,220,231,.42)",4);
      txt("“",142,738,"112px Georgia, serif","rgba(137,151,236,.72)");
      const fit=fitDream(dream.dream_text,770,520),blockH=fit.lines.length*fit.lineH;let yy=690+Math.max(0,(520-blockH)/2)+fit.size*.78;for(const ln of fit.lines){txt(ln,178,yy,"800 "+fit.size+"px Inter, Arial","#FCFDFE");yy+=fit.lineH}

      /* Human identity */
      line(84,1320,996,1320,"rgba(255,255,255,.07)",2);
      txt("DREAMED BY",86,1370,"900 11px Inter, Arial","#718A96");const name=String(dream.nickname||"Anonymous"),nameSize=fitName(name,430);txt(name,86,1422,"800 "+nameSize+"px Space Grotesk, Inter, Arial","#F1F5F6");
      pill(708,1368,288,52,"rgba(7,18,25,.72)","rgba(91,211,223,.14)");x.beginPath();x.arc(736,1394,6,0,Math.PI*2);x.fillStyle="#6EDCE5";x.fill();txt("PERMANENT IN THE ARCHIVE",852,1400,"900 11px Inter, Arial","#AFC8CF","center");

      /* Four worlds, reduced to a signature strip. */
      txt("ONE DREAM · FOUR WORLDS",86,1502,"900 10px Inter, Arial","#71838E");
      const labels=[["MAP","#62DBE6"],["WALL","#A995EF"],["CHAIN","#7DDCCB"],["PUZLE","#A8E66E"]];
      labels.forEach((it,i)=>{const px=84+i*229;rr(px,1530,210,78,18,"rgba(5,12,19,.60)","rgba(255,255,255,.065)");x.beginPath();x.arc(px+28,1569,7,0,Math.PI*2);x.fillStyle=it[1];x.fill();txt(it[0],px+53,1575,"900 14px Inter, Arial","#C9D5DA")});

      /* Footer */
      line(84,1670,996,1670,"rgba(255,255,255,.07)",2);
      txt("ONE PERSON. ONE DREAM. ONE PERMANENT PLACE.",84,1730,"900 13px Inter, Arial","#D8E0E4");
      txt("ONEDREAMEACH.COM/DREAM/"+String(dream.dream_number||padded),84,1782,"900 13px Inter, Arial","#7FDDE5");
      txt("SAVE · SHARE · KEEP IT MOVING",996,1782,"900 10px Inter, Arial","#74858F","right");
